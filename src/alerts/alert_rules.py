...
alert_rules.py
──────────────
Rule-based alert engine for the credit risk monitoring system.

Alerts are classified as:
  - CRITICAL : immediate action required (escalate, consider retraining)
  - WARNING  : monitor closely (investigate root cause)
  - INFO     : informational only

Each rule maps a monitoring metric to a threshold and a severity level.
All active rules are evaluated every month and results are stored in an
AlertReport for dashboard rendering and logging.
"""
...
def _should_retrain(alerts: list[Alert], metrics: dict) -> bool:
    """
    Retrain trigger:
      - Any critical drift alert (PSI > 0.25)
      - AUC drops below 0.72
      - KS drops below 0.28
      - 3 or more critical alerts in a single month
    """
...
def evaluate_alerts(
    ...
    """
    Evaluate all alert rules against the current month's metrics.

    Parameters
    ----------
    metrics      : Dict with keys matching metric names in RULES.
                   Expected keys: auc, ks, brier, calibration_error,
                   observed_default_rate, el_m, mean_ep_ratio,
                   psi_score, mean_pd_shift.
    month_label  : e.g. "2018-09"
    feature_psi  : Per-feature PSI dict for generating individual drift alerts.
    """
