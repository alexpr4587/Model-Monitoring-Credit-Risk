...
──────────────────────
Model performance monitoring for the credit risk system.

Tracks per-month:
  - ROC-AUC   : discriminatory power
  - KS Stat   : Kolmogorov-Smirnov separation (banking standard)
  - Brier     : calibration quality / probability accuracy
  - PR-AUC    : precision-recall AUC (better for imbalanced classes)

Banking thresholds (industry standard):
  AUC  ≥ 0.75 → Good | 0.70–0.75 → Watch | < 0.70 → Retrain
  KS   ≥ 0.40 → Good | 0.30–0.40 → Watch | < 0.30 → Retrain
  Brier ≤ 0.16 → Good | 0.16–0.22 → Watch | > 0.22 → Investigate
"""
...
def calculate_ks(y_true: np.ndarray, y_prob: np.ndarray) -> float:
    ...
    KS = max | CDF_defaulters(t) - CDF_non_defaulters(t) |
    """
...
def calculate_performance(
    ...
    """
    Compute all performance metrics for one production month.

    Parameters
    ----------
    y_true      : Observed default labels (0/1).
    y_prob      : Predicted PD (probability of default).
    month_label : e.g. "2018-03" for tracking.
    """
...
def performance_timeseries(results: list[PerformanceResult]) -> pd.DataFrame:
    """Convert a list of monthly PerformanceResult objects to a DataFrame."""
