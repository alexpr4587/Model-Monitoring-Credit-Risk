...
──────────────────────
Orchestrates the full monthly monitoring run.

Flow:
  load models
      ↓
  load baseline (training snapshot for PSI reference)
      ↓
  for each production month:
      load data
      predict (PD, LGD, EAD, approval)
      calculate PSI (data drift)
      calculate prediction drift
      calculate model performance (AUC, KS, Brier)
      calculate calibration
      calculate portfolio metrics (EL, EP, approval_rate, default_rate)
      evaluate alert rules
      save results
      ↓
  export monitoring_results.csv + alert_history.csv
"""
...
# ── path setup ─────────────────────────────────────────────────────────────────
# File lives at:  <project_root>/src/pipeline/monitoring_pipeline.py
...
# ── custom unpickler ──────────────────────────────────────────────────────────
# Models saved from Jupyter notebooks have their classes recorded as belonging
# to '__main__' instead of 'credit_risk_pipeline'. This remaps them on load.
...
class _CreditRiskUnpickler(pickle.Unpickler):
    """Remaps __main__.XYZ → credit_risk_pipeline.XYZ on deserialisation."""
...
def _safe_load(path: Path):
    """Drop-in replacement for joblib.load() that handles __main__ remapping."""
    try:
...
class ModelRegistry:
    """
    Loads and caches all trained model artifacts from the models/ directory.

    Expected artifacts:
      pd_preprocessor.pkl, pd_model.pkl
      lgd_preprocessor.pkl, lgd_model.pkl
      ead_preprocessor.pkl, ead_model.pkl
    """
...
# ── synthetic predictors (used when models are not yet loaded) ─────────────────
...
def run_monthly(
    ...
    """
    Execute all monitoring checks for a single production month.

    Returns a dict with:
      psi_report, perf_result, calib_result, pred_drift_result,
      portfolio_result, alert_report
    """
    ...
    # ── 4. Model performance (requires labels) ────────────────────────────────
...
def run_pipeline(
    ...
    """
    Run the full monitoring pipeline across all production months.

    Returns a dict of DataFrames ready for the dashboard.
    """
    log.info("=" * 60)
