...
scheduler.py
────────────
Production ingestion architecture — two responsibilities:

1. DATA INGESTION (run monthly via cron)
   Generates the current month's production batch, pushes it to S3,
   then triggers the monitoring pipeline.

2. S3 INTEGRATION
   All production CSVs and monitoring results are stored in S3.
   The Streamlit dashboard reads directly from S3 — no local state.
...
3. Set your bucket name:
      export S3_BUCKET=your-credit-risk-monitoring-bucket

4. Cron entry (runs on the 1st of each month at 06:00):
      0 6 1 * * /path/to/anaconda3/bin/python /path/to/src/pipeline/scheduler.py --run-month >> /var/log/credit_risk_monitor.log 2>&1
...
def get_s3_client():
    """Returns a boto3 S3 client. Raises ImportError if boto3 not installed."""
    try:
        import boto3
...
class S3Store:
    """
    Thin wrapper around S3 for uploading and downloading monitoring artifacts.
    Falls back to local filesystem if S3_BUCKET is not configured or boto3
    is not installed — so the system works locally without AWS credentials.
    """
    ...
    def sync_monitoring_results(self, results_dir: Path) -> None:
        """Push all monitoring result files to S3."""
    ...
    def pull_production_data(self, local_prod_dir: Path) -> list[Path]:
        """
        Pull all production CSVs from S3 to local disk.
...
def _current_month_idx(now: datetime = None) -> tuple[int, int, int]:
    """
    Maps the current calendar date to (year, month, month_idx).
    month_idx is 1-based position within the simulation window.
    """
...
def run_monthly_ingestion(dry_run: bool = False) -> None:
    ...
    Steps:
      ...
      2. Generate production batch (convex mix)
      3. Upload CSV to S3: production/YYYY_MM.csv
      4. Pull any new production files from S3 to local
      5. Run full monitoring pipeline
      6. Push monitoring results back to S3
    """
    log.info("=" * 60)
...
def load_results_from_s3(cache_dir: Path = None) -> dict:
    """
    Load monitoring results, preferring S3 over local cache.
    Used by dashboard/app.py when S3 is configured.

    Falls back to local files if S3 is unavailable.
    """
    import pandas as pd
    import json
...
if __name__ == "__main__":
    ...
    elif args.sync_from_s3:
        log.info("Pulling production files from S3...")
        pulled = store.pull_production_data(PROJECT_ROOT / "data" / "production")
